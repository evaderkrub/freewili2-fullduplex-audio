...
// ---- Frame streaming: ONE RAMWR/CS session per frame, strips fed by DMA. ----
// This is the write shape the reference driver proved on this panel (whole-frame
// DMA with CS held low); we just chunk the source into strips so decode can fill
// one buffer while DMA drains the other. No CS/DC edges occur between strips.

static int stream_dma_chan = -1;   // claimed once on first stream, held for program lifetime

// paint_bars cache — also invalidated by st7796_fill_screen
static int painted_w = -1, painted_h = -1;
static uint16_t painted_bar = 0;

void st7796_stream_begin(int x0, int y0, int w, int h) {
    if (stream_dma_chan < 0)
        stream_dma_chan = dma_claim_unused_channel(true);
    st7796_set_window((uint16_t)x0, (uint16_t)y0,
                      (uint16_t)(x0 + w - 1), (uint16_t)(y0 + h - 1));
    push_begin();   // CS low + RAMWR + DC high; session stays open until stream_end
}

void st7796_stream_strip(const uint8_t *bytes, size_t n) {
    // Wait for the previous strip to finish before retargeting the channel; this
    // is also what hands the previous buffer back to the decoder.
    dma_channel_wait_for_finish_blocking(stream_dma_chan);
    dma_channel_config c = dma_channel_get_default_config(stream_dma_chan);
    channel_config_set_transfer_data_size(&c, DMA_SIZE_8);
    channel_config_set_dreq(&c, spi_get_dreq(LCD_SPI, true));
    channel_config_set_read_increment(&c, true);
    channel_config_set_write_increment(&c, false);
    dma_channel_configure(stream_dma_chan, &c,
                          &spi_get_hw(LCD_SPI)->dr,   // write: SPI TX FIFO
                          bytes,                       // read: strip buffer
                          n, true);                    // start now
}

void st7796_stream_end(void) {
    dma_channel_wait_for_finish_blocking(stream_dma_chan);
    // DMA done != shifted out: drain the SPI before raising CS.
    while (spi_get_hw(LCD_SPI)->sr & SPI_SSPSR_BSY_BITS) tight_loop_contents();
    push_end();     // CS high with NOP guard
}

void st7796_blit_full(const uint16_t *fb, bool wait) {
    st7796_set_window(0, 0, ST7796_W - 1, ST7796_H - 1);
    // fb is pre-byte-swapped (big-endian on the wire); send raw, MSB-first.
    push_pixels((const uint8_t *)fb, (size_t)ST7796_W * ST7796_H * 2);
    (void)wait;
}

void st7796_paint_bars(int w, int h, uint16_t bar_be) {
    if (painted_w == w && painted_h == h && painted_bar == bar_be) return;
    // Paint the letterbox bars, each as ONE block write — a repeated line streamed
    // inside a single RAMWR session. (Per-row writes are a shape this panel drops;
    // block windows are the proven shape.)
    int y0 = (ST7796_H - h) / 2;
    static uint16_t line[ST7796_W];
    for (int x = 0; x < ST7796_W; x++) line[x] = bar_be;
    int bot = y0 + h;                        // rows bot..ST7796_H-1
    if (y0 > 0) {
        st7796_set_window(0, 0, ST7796_W - 1, y0 - 1);
        push_begin();
        for (int y = 0; y < y0; y++)
    ...
    if (bot < ST7796_H) {
        st7796_set_window(0, bot, ST7796_W - 1, ST7796_H - 1);
        push_begin();
        for (int y = bot; y < ST7796_H; y++)
            spi_write_blocking(LCD_SPI, (const uint8_t *)line, sizeof line);
        push_end();
    }
    // Side bars unhandled: clips are always full panel width (w == 480).
    painted_w = w; painted_h = h; painted_bar = bar_be;
}
...
void st7796_fill_screen(uint16_t color_be) {
    ...
    for (int y = 0; y < ST7796_H; y++)
        spi_write_blocking(LCD_SPI, (const uint8_t *)line, sizeof line);
    push_end();
    painted_w = painted_h = -1;   // bars cache no longer reflects the GRAM
}

void st7796_bars_invalidate(void) {
    painted_w = painted_h = -1;
}
...
void st7796_blit_centered(const uint16_t *img, int w, int h, uint16_t bar_be, bool wait) {
    st7796_paint_bars(w, h, bar_be);
    int x0 = (ST7796_W - w) / 2, y0 = (ST7796_H - h) / 2;
    st7796_set_window(x0, y0, x0 + w - 1, y0 + h - 1);
    push_pixels((const uint8_t *)img, (size_t)w * h * 2);
    (void)wait;
}

void st7796_wait_blit(void) { /* synchronous blocking SPI; nothing to wait for */ }
